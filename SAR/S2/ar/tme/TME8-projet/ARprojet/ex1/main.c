...
void simulateur(long cseed)
{
	...
	int *pairs_valeurs;//对等节点
	struct pair *tableau_pairs;
	int nombre_pairs;  //对等节点总数
    int nombre_clefs;  //唯一键总数
	int nombre_clefs_exposant; //键空间大小的指数 2的多少次方

	if (dht_aleatoire) { //判断随机生成还是根据TD生成
		...
		nombre_clefs = 1 << nombre_clefs_exposant;  //2的 nombre_clefs_exposant次方，将1向左移动 nombre_clefs_exposant位
