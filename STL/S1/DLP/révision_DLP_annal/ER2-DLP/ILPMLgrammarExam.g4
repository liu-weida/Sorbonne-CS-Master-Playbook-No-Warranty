...
// construction d'objets
    ...
    | '$' discrim=STRING '(' args+=expr? (',' args+=expr)* ')' # Sum
...
// affectation d'un attribut d'objet
    ...
    | 'match' sum=expr 'with' tag=STRING '(' vars+=IDENT? (',' vars+=IDENT)* ')'
      'then' consequence=expr 'else' alternant=expr # Filter
